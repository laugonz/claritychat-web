...
Each language contains the homepage, guide index, seven guides, privacy, terms and support: 72 static pages, including 42 guides. A native header selector links to the equivalent page in every language. Navigation, image descriptions, captions, metadata and structured data are translated. Five product screenshots per language now cover the homepage, guides and social preview. English retains the original assets; Spanish, French, German and Italian use published App Store screenshots, and Polish uses existing native captures from ClarityChat-deck. The assets retain the actual app pixels, including existing English severity badges such as High/Medium and chart abbreviations. No screenshot UI was repainted.

There are 432 source messages per catalog. Public copy was machine-drafted and then edited for meaning, terminology, brand preservation and natural wording; no professional native-speaker certification is claimed. Translation happens at build time, with no visitor-side translation service. See [implementation and verification notes](docs/website-languages-20260913.md) and [screenshot provenance](docs/screenshot-sources-20260913.json).
